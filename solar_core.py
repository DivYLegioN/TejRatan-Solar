...
class PPAPartner:
    def __init__(self, name: str, monthly_units: float, kv_level: int,
                 ...
                 custom_tariff: float = None, custom_fixed_rate: float = None):
        self.name = name
        ...
        # NEW -- "OTHER" category support. When category == "OTHER" (see
        # MPPBTariffConfig.CATEGORIES / CATEGORY_LABELS below), this partner
        # is NOT looked up in MPPBTariffConfig.TARIFF / FIXED_CHARGE_RATE.
        # Instead the caller supplies the government tariff (Rs/unit) and
        # fixed-charge rate (Rs/kVA) directly, e.g. for a consumer type/
        # tariff slab that isn't one of the standard HV3.1/HV3.2/LV4.1 rows.
        # BillingEngine.gov_bill() / original_gov_bill() check for this and
        # use these values in place of a MPPBTariffConfig lookup.
        self.custom_tariff = custom_tariff
        self.custom_fixed_rate = custom_fixed_rate
        if category == "OTHER" and (custom_tariff is None or custom_fixed_rate is None):
            raise ValueError(
                "category='OTHER' requires custom_tariff (Rs/unit) and "
                "custom_fixed_rate (Rs/kVA) to be supplied."
            )
...
class BillingEngine:
    ...
    def gov_bill(partner: PPAPartner, gov_units: float) -> float:
        ...
        If partner.category == 'OTHER', uses partner.custom_tariff /
        custom_fixed_rate instead of a MPPBTariffConfig lookup."""
        if partner.category == "OTHER":
            government_tariff = partner.custom_tariff
            fixed_rate = partner.custom_fixed_rate
        else:
    ...
    def original_gov_bill(partner: PPAPartner) -> float:
        ...
        MPPB formula, applied to the partner's full monthly_units. If
        partner.category == 'OTHER', bypasses the category/connection
        lookup and bills at partner.custom_tariff / custom_fixed_rate
        directly via MPPBBillingEngine.mppb_bill_custom()."""
        if partner.category == "OTHER":
            result = MPPBBillingEngine.mppb_bill_custom(
                gov_units=partner.monthly_units,
                mppb_tariff=partner.custom_tariff,
                fixed_rate=partner.custom_fixed_rate,
...
class MPPBTariffConfig:
    ...
    CATEGORY_LABELS = {
        ...
        "LV_4.1": "LV 4.1",
        # "OTHER" is a manual-entry category, deliberately NOT added to
        # CATEGORIES above (it has no fixed connection list and skips
        # validate_category_connection / TARIFF / FIXED_CHARGE_RATE lookup
        # entirely -- see PPAPartner.custom_tariff/custom_fixed_rate and
        # BillingEngine.gov_bill()/original_gov_bill()). Listed here only
        # so UI layers can show it in a dropdown label.
...
class MPPBBillingEngine:
    ...
    @staticmethod
    def mppb_bill_custom(gov_units: float, mppb_tariff: float, fixed_rate: float,
                          contract_demand: float, duty_pct: float = None,
                          fppas_pct: float = None, other_charges: float = 0.0) -> Dict:
        """Same formula as mppb_bill(), but for the 'OTHER' / manual-entry
        category: mppb_tariff (Rs/unit) and fixed_rate (Rs/kVA) are supplied
        directly by the caller instead of being looked up in
        MPPBTariffConfig by (category, connection)."""
        energy_charges = MPPBBillingEngine.mppb_energy_charges(gov_units, mppb_tariff)
        fppas_amt = MPPBBillingEngine.fppas(energy_charges, fppas_pct)
        duty_amt = MPPBBillingEngine.mppb_electricity_duty(gov_units, mppb_tariff, duty_pct)
        fixed_amt = MPPBBillingEngine.fixed_charge(contract_demand, fixed_rate)
